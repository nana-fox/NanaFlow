...
| 闸门 | 结果 | 证据 |
|---|---|---|
| 全量测试 | 205/205，0 failures | 最新 xcresult 汇总；新增主窗顶部悬停区和低进度圆角填充回归，并保留此前全部回归 |
| xcresult | 可复查 | `/Users/nio/Library/Developer/Xcode/DerivedData/NanaFlow-cmxejewtsqtmuyfzqwgmewnfaovb/Logs/Test/Test-NanaFlow-2026.08.29_10-05-49-+0800.xcresult` |
| 覆盖率 | 全 App 75.43%（8530/11309） | 同一 xcresult；整体 83.03%（15092/18176） |
...
| Analyze | `ANALYZE SUCCEEDED` | 最新 Debug/arm64 串行 `xcodebuild analyze -quiet` 返回 0 且无诊断 |
| Release | 本地通用包可运行，主 App 与 Widget 均为 arm64 + x86_64 | `dist/NanaFlow.app`、`dist/NanaFlow-macOS-universal.zip`；深度签名校验通过，ZIP SHA-256 为 `faa0bfd08c26d5a085be4ba6433b83207d26db97c21718dd59a1601ffd6ac7e7` |
...
5. **本地 Release 与正式签名边界**：当前 `dist/NanaFlow.app` 已以 ad-hoc 签名通过 `codesign --verify --deep --strict`，并由 Computer Use 实际启动到 25:00 主窗；它适合本机功能与视觉验收。App Group、iCloud、Calendar、通知、浏览器自动化权限和 Widget Gallery 仍需 Apple Developer 证书及 provisioning profile，不能把本地可运行包冒充正式分发签名。
