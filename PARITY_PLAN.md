...
| 闸门 | 结果 | 证据 |
|---|---|---|
| 全量测试 | 214/214，0 failures | `/tmp/nanaflow-audit-final-test.log`；包含五个主窗页面共享可见窗框高度与控件名称回归 |
| xcresult | 可复查 | `/tmp/nanaflow-audit-final.8QSx7c/Logs/Test/Test-NanaFlow-2026.08.31_09-22-16-+0800.xcresult` |
| 覆盖率 | 全 App 75.26%（7868/10454） | 同一 xcresult；视图尺寸与可访问性断言已执行 |
...
| Analyze | `ANALYZE SUCCEEDED` | 干净 Release DerivedData、0 warning，日志 `/tmp/nanaflow-audit-final-analyze.log` |
| Release | 本地通用包可运行，主 App 与 Widget 均为 arm64 + x86_64 | `dist/NanaFlow-7d911b9-macOS-universal.zip`；解压后深度签名校验通过，ZIP SHA-256 为 `2351c474657c6f4949ef4a77ec8f6ba4cbe615411d03a652a508080e892dab20` |
...
5. **本地 Release 与正式签名边界**：当前 `dist/NanaFlow-7d911b9-macOS-universal.zip` 内的 App 已以 ad-hoc 签名通过 `codesign --verify --deep --strict`；它适合本机功能与视觉验收。App Group、iCloud、Calendar、通知、浏览器自动化权限和 Widget Gallery 仍需 Apple Developer 证书及 provisioning profile，不能把本地可运行包冒充正式分发签名。
